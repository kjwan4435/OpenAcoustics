        ...
        if (len(fullPacket) != buffer_size): #except for realtime streaming
            ...
            if (header[0:5] == "SUBID"):
                ...
                if (timestamp == "-"):  # not yet started
                    ...
                    sz = int(len(fullPacket)/4)
                    soundFile = np.empty([sz, 2], dtype=np.int16)
                    # bottomSoundFile = np.empty([sz, 1], dtype=np.int16)
                    # topSoundFile = np.empty([sz, 1], dtype=np.int16)
                    for index in range(0, sz):
                        i = index * 4
                        value_top = (fullPacket[i+1] << 8) + fullPacket[i]    # little endian
                        value_bottom = (fullPacket[i+3] << 8) + fullPacket[i+2]
                        if value_top > 32767:  # If value exceeds max int16
                            value_top -= 65536
                        if value_bottom > 32767:  # If value exceeds max int16
                            value_bottom -= 65536
                        value_top = np.int16(value_top)
                        value_bottom = np.int16(value_bottom)
                        soundFile[index][0] = value_top
                        soundFile[index][1] = value_bottom
                        # topSoundFile[index][0] = value_top
                        # bottomSoundFile[index][0] = value_bottom
                    wav.write('audio' + trial_id + '.wav', int(fileFreq), soundFile)
                    # wav.write('audio_top' + trial_id + '.wav', int(fileFreq), topSoundFile) 
                    # wav.write('audio_bottom' + trial_id + '.wav', int(fileFreq), bottomSoundFile)
